...
func StartApiServer(logger *zap.Logger, startupLogger *zap.Logger, protojsonMarshaler *protojson.MarshalOptions, protojsonUnmarshaler *protojson.UnmarshalOptions, config config.Config, version string, sessionRegistry SessionRegistry, sessionCache SessionCache, statusRegistry StatusRegistry, tracker Tracker, router MessageRouter, streamManager StreamManager, metrics Metrics, pipeline *Pipeline, presignService *PresignService) *ApiServer {
	...
	grpcGatewayRouter.HandleFunc("/ws", NewSocketWsAcceptor(logger, config, sessionRegistry, sessionCache, statusRegistry, tracker, metrics, protojsonMarshaler, protojsonUnmarshaler, pipeline)).Methods(http.MethodGet)
...
func traceApiAfter(ctx context.Context, logger *zap.Logger, fullMethodName string, fn func(clientIP, clientPort string) error) {
}
